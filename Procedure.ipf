...
// The code is for (1) simulating the responses of the T2 and T3 neurons in response to square wave inputs (see article for details), (2) computing biophysically realistic spike trains and discreet stochastic synaptic responses for use in the 
//  in the XPPAUT DA neuron model. 
// (1) Simulating the responses of the T2 and T3 neurons in response to square wave inputs.   The input (V(t) in the article is  the wave "input2"  in the code. The activity of T2 and T3 neurons are the "out[0]" and "out[6]" arrays 
//   in the 2D wave "out"
// The function " myInt()"  intergates the ODEs defining the behavior or the T2 and T3 cells, it stimulates responses to a single square wave (or other) input, given in "input2".    This function is called by the function "doit()". The "input2"
// is constructed in "doit()" 
// The function "doit" is sued to set the global variables including the length of simulated trails,  constrcut the square pulse  input wave, and calculates the weighted sum of the T2 and T3 cells activity which is stored on thewaves 
//  "da_w_1_ n"  where is the trial number. This output is shown in Figure 3 of the article as the net GABA output.
//  The functions "spikeTrainExp(wns)" and "spikeTrainExp2(wns)" compute the stochastic synaptic input to DA neurons from T2 and T3 cells.  These are stored in the waves 't4_n' and 't5_n' and these are the 2 of the 3 inputs to the CELLMOD.ode
//  The "spikeTrainExp(wns)" and "spikeTrainExp2(wns)" are called by "discr()"  which also calls "doit()" and generates the whole series of  responses to increasing square input amplitudes. 
